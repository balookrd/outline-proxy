...
- **Home screen header banner border contrast on OLED displays.** In dark theme, the outline border around the brand banner panel now uses higher contrast `colorScheme.outline` (0.85 opacity) rendered on an overlay layer above the banner bitmap, preventing the raster image from obscuring border pixels and ensuring the rounded container boundary remains crisp and visible on OLED screens such as Honor Magic V6.
