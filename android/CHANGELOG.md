...
- **Compact split-tunnel mode selector and streamlined layout.** Replaced the bulky vertical 3-row radio card with a sleek horizontal segmented selector ("All", "Selected", "Bypass"), and removed the redundant top banner for Google/Gemini in favour of the existing inline hint under the Gemini list entry, freeing up substantial vertical screen space for the application list.
...
- **Prevent false auto-resume of paused VPN when screen is off on Wi-Fi.** Fixed a bug where a VPN paused on a trusted Wi-Fi network would spontaneously resume overnight (while the phone was idle in Doze mode) and immediately pause again when the user opened the app:
  - `AutomationPolicy.decideWifiChange()` now separates network presence (`isOnWifi`) from SSID availability: when connected to Wi-Fi with an unknown/redacted SSID (due to Android background location limits when screen is off), the existing pause state is strictly preserved rather than misinterpreting a hidden SSID as having left the trusted network.
  - Added a 3-second debounce to Wi-Fi disconnection (`onLost`) in `OutlineVpnService`, preventing transient network drops, roaming between 2.4/5 GHz bands, or DHCP lease renewals from falsely triggering tunnel revival.
  - Added unit tests covering background standby and hidden SSID conditions.
