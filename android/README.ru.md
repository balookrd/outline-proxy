...
переиспользуется без изменений; Android предоставляет современный интерфейс
Material Design 3 (Material You) и слой `VpnService`.
...
> подписка на конфиг по URL, полный дизайн **Material Design 3** (динамические
> темы Material You на Android 12+, тематические монохромные иконки на Android 13+),
> новая айдентика (щит защиты с динамичным туннелем скорости), подписанные release-сборки,
> единая кнопка connect/disconnect с индикатором подключения, удержание туннеля при
> убийстве / ребуте / OEM-чистке, foreground-уведомление с именем профиля и навигация
> системной кнопкой «назад». Весь Rust-стек (включая quinn + h3) кросс-компилируется
> под NDK r29, а Gradle/Kotlin-приложение собирается (debug-APK, минифицированный
> release-APK, зелёные JVM-юнит-тесты). Приложение запускалось на **эмуляторе**
> (на прежнем мосте tun2proxy, ныне заменённом нативным движком); на **реальном железе**
> прогнан только экран чеклиста keep-alive и его вендорские intent'ы. Трафик через
...
android/
  ...
  app/             # Android-приложение (Gradle, Kotlin, Compose)
    ...
    src/test/java/com/outline/proxy/
      ExternalControlTest.kt, KeepAlivePolicyTest.kt, AutomationPolicyTest.kt,   # JVM-юнит-тесты
...
Генерируемый профиль несёт ещё и `[outline.probe]` — DNS-пробу плюс TLS-рукопожатия
через туннель, повторяя группу `main` самого парка. Без них у клиента нет сигнала
здоровья, независимого от трафика: помеченный мёртвым аплинк оживает только от
данных, прошедших по нему, поэтому потеря разом всех аплинков (смена SIM, мёртвая
сота) читалась как «no link» до ручного переподключения. Интервал проб — 60 с
против парковых 10 с: на телефоне разница измеряется батареей, а на канале
edge-класса — той самой полосой, которой не хватает туннелю.
...
### Бюджет дозвона на медленных каналах

В генерируемом конфиге секции `[dial]` нет, поэтому ядро ограничивает каждый
свежий дозвон носителя своим дефолтом в 10 с. Для Wi-Fi и LTE это нормально, а
на соте edge-класса именно этот бюджет и есть причина отказа: на 2G round trip
доходит до секунды, одна цепочка сертификатов вытекает несколько секунд, бюджет
истекает посреди рукопожатия, каждая попытка засчитывается отказом, а повторы
съедают ту полосу, которой рукопожатию не хватило, — туннель числится поднятым и
не несёт ничего.

Поэтому приложение подставляет секцию само, исходя из того, что платформа
сообщает о канале, на который туннель встаёт (`DialTimeout`): сотовому каналу с
оценкой до 200 кбит/с достаётся 60 с, до 2 Мбит/с — 30 с, всё быстрее остаётся с
дефолтом. Wi-Fi и Ethernet не расширяются никогда: медленный Wi-Fi — это обычно
медленный backhaul, где рукопожатие всё равно укладывается в пару round trip'ов,
а расширение границы только замедлило бы failover. Канал, который платформа ещё
не охарактеризовала, не трогается — «нет данных» не равно «2G».

Конфиг, объявляющий `[dial]` самостоятельно, не изменяется: явное значение от
оператора важнее этой догадки.

Бюджет **следует за каналом**, а не фиксируется на момент подключения. Сервис
следит за сетью, по которой идёт, — в том числе через `onCapabilitiesChanged`,
которым на деле и приходят хендовер между сотами, смена поколения
(2G ↔ 3G ↔ LTE ↔ 5G) и, на многих устройствах, переключение SIM для данных:
объект `Network` остаётся прежним, а свойства под ним меняются. Переход с Wi-Fi в
2G-соту расширяет границу, обратный переход возвращает дефолт (оставлять 60 с на
быстром канале — значит без нужды замедлять failover), а ядро подхватывает новое
значение на следующем дозвоне; рукопожатия, уже начатые, доживают со своим
прежним дедлайном. Ничего не разрывается, а повторно та же оценка не
применяется, поэтому колебание уровня сигнала ничего не стоит.

Поколения раскладываются предсказуемо: GPRS и EDGE оцениваются в 200 кбит/с и
ниже — им достаётся 60 с; UMTS, слабые HSPA или LTE попадают под 2 Мбит/с — им
30 с; всё, что здоровее, остаётся с дефолтом. Поскольку не всякая прошивка
переотдаёт оценку полосы при смене одной лишь радиотехнологии, бюджет
дополнительно перепроверяется на уже существующем двухсекундном тике сервиса —
это чтение capabilities и сравнение, бесплатное, когда ничего не изменилось.

### Что главный экран говорит о канале

Под статусом туннеля карточка показывает сеть, по которой туннель реально идёт:
`Wi-Fi · 20 ms` или `LTE · 4.2 s`. Два факта, каждый из которых либо измерен,
либо назван платформой; любой из них не подставляется наугад, а просто
опускается, если неизвестен:

- **Что за канал.** Wi-Fi и Ethernet называют себя сами. Сотовый называет
  радиотехнологию (2G / 3G / LTE / 5G), если приложению разрешено её читать, и
  «Cellular» в противном случае. Это именно та радиосеть, что реально несёт
  данные (`dataNetworkType`), и она регулярно расходится с иконкой в шторке:
  иконка обычно отражает покрытие 5G или NSA-якорь с поддержкой 5G, так что «5G»
  наверху и `LTE` здесь — норма, а не баг.
- **Во что канал обходится туннелю** — последний round-trip, измеренный ядром на
  реальном дозвоне.

Ничего вычисленного не показывается. В более ранней версии выводились ещё оценка
полосы от платформы и класс скорости, посчитанный из неё, — оба убраны: прошивки
эту оценку выдумывают (одно устройство сообщало 14 кбит/с на LTE-соте с полным
сигналом, которая прекрасно несла трафик), а подпись, посчитанная из неверного
числа, — это то же неверное число, только с уверенным видом. Оценка по-прежнему
читается — по ней выбирается бюджет дозвона, пока нечего измерять, — просто не
попадает на экран.

Round-trip, дотянувшийся до бюджета дозвона, тоже отбрасывается. Это дозвон,
у которого кончилось время, и число, которое он даёт, — это бюджет, а не канал:
первая проба после подключения выдала ровно «10.0 s» при бюджете в 10 секунд.
Лучше не показать ничего, чем показать таймаут под видом измерения.

Для названия радиотехнологии нужен `READ_PHONE_STATE`. Само приложение его не
запрашивает: он стоит в чеклисте **Keeping Alive** рядом с остальными грантами и
помечен как необязательный — ни одно решение туннеля от него не зависит, бюджет
дозвона в любом случае выбирается по оценке полосы, и отказ стоит ровно одной
подписи.
