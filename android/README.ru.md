...
Выбор делается **один раз, при старте туннеля** — ядро читает бюджет на старте и
держит его до конца жизни процесса. Переход с Wi-Fi в 2G-соту его не расширит,
для этого нужно переподключение.
