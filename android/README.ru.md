...
которым на деле и приходят хендовер между сотами и, на многих устройствах,
переключение SIM для данных: объект `Network` остаётся прежним, а свойства под
ним меняются. Переход с Wi-Fi в 2G-соту расширяет границу, обратный переход
возвращает дефолт (оставлять 60 с на быстром канале — значит без нужды замедлять
failover), а ядро подхватывает новое значение на следующем дозвоне; рукопожатия,
уже начатые, доживают со своим прежним дедлайном. Ничего не разрывается, а
повторно та же оценка не применяется, поэтому колебание уровня сигнала ничего не
стоит.
