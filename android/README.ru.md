...
`Wi-Fi · ~90 Mbit/s · 20 ms` или `LTE · very slow · ~14 kbit/s · 4.2 s`. Четыре
независимые вещи, и любая из них не подставляется наугад, а просто опускается,
если неизвестна:
...
- **Что за канал.** Wi-Fi и Ethernet называют себя сами. Сотовый называет
  ...
  «Cellular» в противном случае.
- **Насколько он быстр**, словами — из оценки полосы от платформы, по тем же
  порогам, по которым выбирается бюджет дозвона, так что подпись и бюджет всегда
  говорят одно и то же. Названо по скорости и никогда по поколению: смысл в том,
  что канал может быть LTE и при этом медленнее GPRS.
- **Насколько быстрым его считает платформа** — сама оценка.
- **Во что он обходится туннелю** — последняя латентность, измеренная ядром на
  аплинке; это и есть число за «Connected · slow».

Технология и скорость разведены намеренно. Телефон, показывающий 5G в шторке,
тогда как эта строка говорит `LTE · very slow · ~14 kbit/s`, — не противоречие и
не баг: иконка в статус-баре обычно отражает покрытие 5G или NSA-якорь с
поддержкой 5G, а не ту радиосеть, что реально несёт данные (её и сообщает
`dataNetworkType`), и ни одна из них ничего не говорит о перегруженной соте,
выдающей 14 кбит/с. Ровно этот случай строка и призвана показывать.
