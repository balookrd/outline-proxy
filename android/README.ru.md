...
`Wi-Fi · ~90 Mbit/s · 20 ms` или `Cellular · 2G-class · ~120 kbit/s · 4.2 s`.
Три независимые вещи, и любая из них не подставляется наугад, а просто
опускается, если неизвестна:

- **Что за канал.** Wi-Fi и Ethernet называют себя сами. Сотовый называет
  радиотехнологию (2G / 3G / LTE / 5G), если приложению разрешено её читать, а
  иначе откатывается к классу скорости, выведенному из оценки полосы от
  платформы, — по тем же порогам, по которым выбирается бюджет дозвона, так что
  подпись и бюджет всегда говорят одно и то же.
