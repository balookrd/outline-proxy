...
> release-APK, зелёные JVM-юнит-тесты), и обе сборки **запускались на
> эмуляторе** — TUN поднят, Rust-ядро стартовало, хендовер отработан. На
...
- **Проверено на эмуляторе** (Pixel_10, API 37, arm64), debug-сборка: служба
  поднимает TUN, Rust-ядро стартует (SOCKS5 слушает 127.0.0.1:1080, реестр
  аплинков инициализирован), tun2proxy в него соединяется, отрабатывают
  `outline://connect` / `disconnect`, а слежение за несущей сетью проходит
  хендовер Wi-Fi ⇄ сотовая в обе стороны — в `dumpsys connectivity` у
  VPN-агента `underlying{[N]}` переключается между сотовой и Wi-Fi и никогда не
  привязывает саму VPN-сеть.
...
- **Не проверено:** на реальном железе ничего не запускалось, сквозного трафика
  ...
  tunneling ещё ждёт реального прогона. Обработка DNS в tun2proxy на дефолтах и
