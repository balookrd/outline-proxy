...
- Для анализа и доработок сначала используй локальную карту кода из
  `target/code-index/repo-map.md` / `target/code-index/symbols.json`, чтобы не
  читать большие части дерева в контекст. Если индекса нет, он явно устарел или
  задача требует другого среза, перегенерируй его через `scripts/code-index.py`
  (например, `scripts/code-index.py --package outline-transport` для
  локализованной работы). Индекс — навигационная карта: после него открывай
  только релевантные файлы и диапазоны строк. Использование подробнее описано в
  `docs/CODE-INDEX.ru.md`.
