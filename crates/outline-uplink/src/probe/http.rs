...
//!
//! Plain HTTP only. TLS-handshake-only validation against a real product
//! SNI lives in the sibling [`super::tls`] module under its own
//! `[outline.probe.tls]` config block — the two probe families have
//! distinct semantics (one exercises an HTTP exchange, the other a TLS
//! handshake) and distinct metric labels (`probe="http"` vs `probe="tls"`).
...
pub(super) async fn run_http_probe(
    ...
    if url.scheme() != "http" {
        bail!("only http:// probe URLs are supported (use [outline.probe.tls] for TLS)");
    }
