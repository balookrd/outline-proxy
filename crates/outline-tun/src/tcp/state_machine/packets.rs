...
use super::super::{
    ...
    TCP_SERVER_WINDOW_SCALE, build_data_header_custom, build_gso_tcp_header,
...
/// Build the IP/TCP header (no payload) for a single downlink data packet. The
/// payload is written vectored by the writer, so it is not copied into the
/// returned header. Full RFC 793 checksum, computed over the payload chunks
/// (`payload_parts`) without copying them.
pub(in crate::tcp) fn build_flow_data_header(
    state: &TcpFlowState,
    sequence_number: u32,
    acknowledgement_number: u32,
    flags: u8,
...
/// Build the header of a downlink TSO super-segment and its `virtio_net_hdr`.
/// `payload` is up to ~64 KiB spanning many MSS worth of data and is written
/// vectored by the writer, not copied into the header; the kernel splits it
/// into `mss`-sized segments, filling in each segment's sequence number and
...
pub(in crate::tcp) fn build_gso_flow_header(
    state: &TcpFlowState,
    sequence_number: u32,
    acknowledgement_number: u32,
    flags: u8,
    ...
    let options = default_packet_options(state);
    let (header, l3_len, l4_len) = build_gso_tcp_header(
        state.key.version,
        state.key.remote_ip,
        state.key.client_ip,
        state.key.remote_port,
