//! mux.cool (VLESS `Mux` command) frame layout: parse and encode.

use bytes::{BufMut, BytesMut};
use thiserror::Error;

use crate::target::TargetAddr;
use crate::vless::{ATYP_DOMAIN, ATYP_IPV4, ATYP_IPV6};
...
fn parse_target(input: &[u8]) -> Result<(TargetAddr, Network, usize), MuxError> {
    ...
    let (addr, used) = match atyp {
        ...
        ATYP_IPV6 => {
            ...
            (TargetAddr::IpV6(std::net::Ipv6Addr::from(oct), port), 16)
        },
