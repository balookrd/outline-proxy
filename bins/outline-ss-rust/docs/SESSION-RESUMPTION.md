...
> **What "closes" means per carrier.** On the WebSocket carriers (and their h2/h3 variants) the close is free: a Close frame, or the death of the TCP/QUIC stream under it, ends the relay's carrier read and the sequence above starts within milliseconds. **XHTTP packet-up has no such signal** — the downlink GET and each uplink POST are separate requests, and a client that simply stops POSTing is indistinguishable from one that has gone quiet, so a dropped GET only releases the downlink slot for a reattach. The close therefore travels as a header: the client's final POST carries `X-Xhttp-Fin`, which collapses the session's uplink so the relay reads EOF and parks. Our client sends it on both teardown shapes — an explicit `close()` and a plain drop of the transport, which is what the TUN data plane does — but it stays a hint: a client that is killed, or whose network dies, sends nothing, and the fallback is the registry's 180 s idle sweep. Correct, but far too late for a soft switch, which needs the park to exist by the time the redial arrives. Note the ordering constraint: the FIN collapses the uplink, so it must be the session's *last* POST or the tail of the upload is discarded with it.

> **A dirty death parks too.** "Closes" above covers a carrier that *failed*, not just one that ended: a receive error on the relay's carrier read — a collapsed H3/QUIC connection, a reset TCP connection, an H2 stream error — must run this sequence before the error is reported, exactly as a Close frame does. This is not a refinement, it is the whole point: the TUN client starts a carrier migration only when its carrier died dirty (`!closed_cleanly()`), so a park that happens on graceful closes alone is a park no client ever asks for, and a resume request that follows a dirty death can only ever miss. Regression-tested by `ss_resume_hit_after_dirty_carrier_death` / `vless_resume_hit_after_dirty_carrier_death`, which reset the carrier (`SO_LINGER = 0`) instead of closing it.
...
- **The ring belongs to the session, not to the carrier.** It is carried back out of the park on *every* resume hit, including one whose carrier negotiated no v2 at all — a client is free to advertise v2 on one dial and not the next (the wire-handover redial deliberately advertises nothing), and dropping the ring on such a hop would lose it silently for good, so the next carrier that does ask for a replay is answered `REPLAY_TRUNCATED` / `no_ring` for a session that had a ring all along. Allocating a *fresh* ring for a park that carries none still requires the resuming carrier to engage v2.
- **A ring exists only if the dial that started the session asked for one.** The capability advertisement is what allocates it, and the allocation happens once, at upstream-handshake time — so a client that advertises only on its migration redial gets a park with no ring at exactly the moment it needs one. Every client dial that starts a new session should therefore advertise v1 + v2, including pooled/pre-warmed carriers, which are new sessions from the server's point of view. Advertising on a dial that presents no Session ID is free: a resume hit — and therefore a control frame — is impossible on it.
