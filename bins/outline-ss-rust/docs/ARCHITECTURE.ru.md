...
Если две fallback-ручки защищают *неподходящую* поверхность листенера, то carrier-padding укрепляет сами *подходящие* носители. Каждый Shadowsocks-чанк на padded WS / XHTTP-пути оборачивается в кадр `real_len | pad_len | real | pad` (кодек в `crates/outline-wire/src/padding.rs`) до попадания во внешний TLS-record, так что размер зашифрованной записи больше не повторяет размер Shadowsocks-payload — это ломает корреляцию по размеру записей, на которую опираются классификаторы «proxy-inside-TLS» (TLS-in-TLS). Релей декодирует входящие кадры до AEAD-слоя (`handle_tcp_binary_frame`) и фреймит downlink в `ChannelSink`. Per-path (`[padding] paths`), config-синхронизация с клиентом (on-wire-бита нет, по умолчанию выключено), с опциональным idle cover-трафиком на простаивающем соединении. Едет на уровне `Message`, общем для всех носителей WS / XHTTP, поэтому один механизм покрывает SS-over-WS (h1/h2/h3) и SS-over-XHTTP; UDP и raw-QUIC вне scope. Полный справочник: [`../../../docs/PADDING.md`](../../../docs/PADDING.md).
