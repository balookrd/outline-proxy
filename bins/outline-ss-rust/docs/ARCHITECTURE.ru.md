...
Если две fallback-ручки защищают *неподходящую* поверхность листенера, то carrier-padding укрепляет сами *подходящие* носители. Каждый Shadowsocks-чанк — или VLESS-кадр — на padded WS / XHTTP-пути оборачивается в кадр `real_len | pad_len | real | pad` (кодек в `crates/outline-wire/src/padding.rs`) до попадания во внешний TLS-record, так что размер зашифрованной записи больше не повторяет размер проксируемого payload — это ломает корреляцию по размеру записей, на которую опираются классификаторы «proxy-inside-TLS» (TLS-in-TLS). SS-релей декодирует входящие кадры до AEAD-слоя (`handle_tcp_binary_frame`) и фреймит downlink в `ChannelSink`; VLESS-релей (`run_vless_relay`) делает то же вокруг разбора VLESS, пропуская каждое downlink-сообщение через `frame_vless_downlink`. Per-path (`[padding] paths`), config-синхронизация с клиентом (on-wire-бита нет, по умолчанию выключено), с опциональным idle cover-трафиком на простаивающем соединении. Едет на уровне `Message`, общем для всех носителей WS / XHTTP, поэтому один механизм покрывает SS- и VLESS-over-WS (h1/h2/h3) и -over-XHTTP; SS-UDP и raw-QUIC вне scope, но VLESS-UDP — делящий путь с VLESS-TCP — паддится по датаграмме. Полный справочник: [`../../../docs/PADDING.md`](../../../docs/PADDING.md).
