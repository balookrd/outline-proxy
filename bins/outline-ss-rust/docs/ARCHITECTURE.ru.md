...
Независимые ручки делают публичный листенер неотличимым от обычного веб-фронтенда для пассивных сканеров. Две из них — *fallback*-ручки, защищающие неподходящую под маршруты поверхность листенера (L4 SNI dispatch, L7 HTTP fallback); третья, *carrier-padding*, укрепляет форму на проводе у самих подходящих прокси-носителей. Все по умолчанию выключены.
...
1. Читаем в небольшой буфер до тех пор, пока Acceptor не отдаст распарсенный ClientHello (или пока не превышен `max_client_hello_bytes` — некорректно сформированные handshake'и закрываются локально, чтобы не попасть в логи бэкенда).
2. Захваченные байты сохраняются дословно и больше с провода не считываются.
3. Подсмотренный SNI резолвится через предвычисленную таблицу `SniLookup`, построенную один раз при старте из конфига. У таблицы три слоя, опрашиваемых по порядку: exact-`HashMap<String, SniRoute>` (каждая `Exact`-запись из `match_sni` и из `match_sni` каждого бэкенда), wildcard-`Vec<(SniMatcher, SniRoute)>` (каждая `Wildcard { suffix }`-запись в порядке приоритета: сначала локальные, потом бэкенды по порядку объявления) и `catch_all: Option<usize>`, указывающий на catch-all бэкенд. Lookup идёт строго: exact → wildcard-скан → catch-all. `HashMap` схлопывает два прежних O(N)-прохода (`sni_matches_ours` по `match_sni`, потом `find_backend` по `backends`) в одну O(1)-проверку на горячем пути; wildcards остаются O(N), но запускаются только при промахе. Локальные записи вставляются до бэкендовых, поэтому SNI, заявленный обоими списками, выигрывает за `Local`; среди бэкендов выигрывает порядок объявления. Эффект для оператора: exact-запись в любом месте конфига побеждает любой wildcard, независимо от порядка объявления — паттерн «wildcard на апекс остаётся локально, один хост уходит на бэкенд» становится однострочной вырезкой, а не задачей на упорядочивание.
4. Если маршрут резолвится в `Local`, захваченные байты воспроизводятся в локальный TLS-терминатор через обёртку `PrependStream`, которая на `poll_read` сначала отдаёт префикс, а потом проваливается в нижележащий сокет. Дальше handshake продолжается обычным путём, и dispatcher передаёт управление на L7.
5. Если маршрут резолвится в `Backend(idx)`, диспатчер сплайсит на `config.backends[idx]`. Поддерживаются два режима конфигурации:
   - **Один бэкенд** (`backend = "host:port"` на уровне секции): единственный бэкенд неявно catch-all, поэтому все чужие SNI идут к нему.
   - **Несколько бэкендов** (массив `[[sni_fallback.backends]]`): каждая запись содержит `backend`, опциональный `match_sni` (список SNI, роутимых именно сюда) и опциональный `proxy_protocol`. Запись без `match_sni` (или с пустым списком) — catch-all и должна стоять последней; соединение, не подошедшее ни к одному бэкенду, молча сбрасывается. Это позволяет роутить разные домены к разным upstream (например, nginx на `:8443` для одного домена, caddy на `:9443` для другого) без отдельного haproxy впереди.

   В обоих режимах открывается новое TCP-соединение к выбранному upstream, опционально префиксится HAProxy PROXY-protocol v1/v2 заголовок, дописывается захваченный ClientHello и запускается `tokio::io::copy_bidirectional` до закрытия одной из сторон.
...
### Форма на проводе: carrier-padding (`[padding]`)

Если две fallback-ручки защищают *неподходящую* поверхность листенера, то carrier-padding укрепляет сами *подходящие* носители. Каждый Shadowsocks-чанк — или VLESS-кадр — на padded WS / XHTTP-пути оборачивается в кадр `real_len | pad_len | real | pad` (кодек в `crates/outline-wire/src/padding.rs`) до попадания во внешний TLS-record, так что размер зашифрованной записи больше не повторяет размер проксируемого payload — это ломает корреляцию по размеру записей, на которую опираются классификаторы «proxy-inside-TLS» (TLS-in-TLS). SS-TCP-релей декодирует входящие кадры до AEAD-слоя (`handle_tcp_binary_frame`) и фреймит downlink в `ChannelSink`; SS-UDP-релей (`run_udp_relay`) и VLESS-релей (`run_vless_relay`) делают то же по сообщению — декодируют каждую входящую датаграмму до разбора и пропускают каждое downlink-сообщение через `frame_downlink_message`. Per-эндпоинт (`[[endpoint]] padded = true`), config-синхронизация с клиентом (бита на проводе нет, по умолчанию выключено), с опциональным idle cover-трафиком на простаивающем соединении. Едет на уровне `Message`, общем для всех носителей WS / XHTTP, поэтому один механизм покрывает SS- и VLESS-over-WS (h1/h2/h3) и -over-XHTTP, а UDP паддится по датаграмме на любом WS-носителе — и SS-UDP (split: поставьте `padded = true` на его эндпоинт; combined: на общий эндпоинт), и VLESS-UDP. Полный справочник: [`../../../docs/PADDING.md`](../../../docs/PADDING.md).
...
- `protocol`: `http1`, `http2`, `http3`, `socket` (обычные SS-слушатели)
