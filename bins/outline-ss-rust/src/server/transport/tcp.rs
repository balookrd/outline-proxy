...
pub(in crate::server::transport) async fn run_tcp_relay<T: WsSocket>(
    ...
    // enabled (the notice rides a cover frame, so only a padded SS carrier — our
    // own clients — can receive it), else `None` keeps the wire byte-for-byte
    // identical. `injected_monitor` served the retired v4 home, which drove a
    // relayed carrier off the monitor its mesh receiver pinged from an edge
    // THROTTLE_HINT and ran no local tick; every caller now passes `None`, since
    // detection lives on the node that terminates the client.
