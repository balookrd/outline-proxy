...
fn mesh_transport_config() -> Result<quinn::TransportConfig> {
    ...
    // The mesh carries no QUIC datagrams: everything a relay needs rides its own
    // stream. `None` withholds the datagram transport parameter, so a peer that
    // still tries to send one (a pre-v5 build with the retired THROTTLE_HINT
    // sender) fails locally on `send_datagram` instead of putting bytes on the
    // wire for us to buffer and drop. quinn's default *enables* receiving, so
