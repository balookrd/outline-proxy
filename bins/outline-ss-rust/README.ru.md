...
**Наблюдаемость неудачных peek'ов.** Поток, из которого так и не удалось получить ClientHello, учитывается в `outline_ss_sni_peek_failed_total{reason=...}` по четырём корзинам:
