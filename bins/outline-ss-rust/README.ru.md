...
| `tuning.udp_nat_max_entries_per_user` | Per-user cap на число живых UDP NAT-записей, применяется поверх `udp_nat_max_entries` (по умолчанию `0` на всех профилях — выключен). Без него один аутентифицированный пользователь, расходящийся веером по множеству адресов, может занять всю таблицу и не дать остальным арендаторам открывать новые адреса до тех пор, пока не сработает idle-эвикция. Датаграммы к новым адресам пользователя, уже выбравшего свою долю, отбрасываются (тоже считаются в `outline_ss_udp_nat_capacity_dropped_total`); его живые записи продолжают работать, а слот освобождается сразу после idle-эвикции одной из них. Включай на мультитенантных развёртываниях со значением с запасом относительно числа одновременных адресов у легитимного клиента |
