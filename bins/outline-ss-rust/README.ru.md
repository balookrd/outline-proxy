...
- **Покрытие:** работает там, где сервер видит реальный IP клиента — SS-over-WS/XHTTP TCP, все несущие VLESS (WS/XHTTP/H3/raw-QUIC) и все raw-QUIC SS (TCP и UDP). SS-UDP-over-WebSocket откатывается к базовому `id` (на relay-слое peer недоступен).
