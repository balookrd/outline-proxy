...
Модель листенеров теперь явная: если не задан ни `listen`, ни `h3_listen`, сервер завершится с ошибкой конфигурации. Поднимаются только те листенеры, которые явно указаны.
...
| `h3_listen` | Опциональный QUIC-слушатель для HTTP/3; должен быть указан явно при включении QUIC |
...
| `[server.h3].alpn` | Список ALPN-протоколов на QUIC-эндпоинте. Поддерживается только `"h3"` (HTTP/3 + WebSocket-over-HTTP/3). По умолчанию `["h3"]` |
| `[server.h3].initial_mtu` | Стартовый размер QUIC-датаграммы, задаётся как размер **UDP-payload** (MTU пути минус 28 для IPv4, 48 для IPv6). Диапазон 1200–1452. Ставить только на узле, где путь меньше стандартных 1500: без этого сервер стартует с 1400, ядро отбивает каждую датаграмму с `EMSGSIZE`, а quinn откатывается на минимум 1200 вместо наибольшего проходящего размера — это ~12% потерь в каждом пакете плюс ретраи на каждом хендшейке. На узле с закреплённым значением MTU discovery также перестаёт пробовать выше него |
...
| `outbound_ipv6_sticky` | Когда включена ротация (`outbound_ipv6_prefix` / `outbound_ipv6_interface`), закрепляет один source-адрес **на каждый upstream destination IP** на `outbound_ipv6_sticky_ttl_secs` вместо выбора нового случайного source на каждый connect. Чинит Cloudflare-сайты (например `claude.ai`), которые на каждый запрос показывают challenge «Verifying you are human» — cookie `cf_clearance` привязан к source IP клиента, поэтому ротируемый source никогда не несёт решённый challenge дальше. Ротация сохраняется между разными destination и по TTL. Ограниченный in-memory кэш (LRU-лимит + TTL на запись); стартовая проба достижимости по-прежнему тестирует случайные source. Включено по умолчанию; без prefix/interface не действует. Поставьте `false`, чтобы вернуть случайный source на каждый connect. По умолчанию: `true` |
| `outbound_ipv6_sticky_ttl_secs` | TTL в секундах для sticky-пинов source. Ставьте выше времени жизни challenge-clearance у origin (у Cloudflare ~30 мин). По умолчанию: `1800` |
| `tuning_profile` | Именованный пресет ресурсных лимитов: `small` / `medium` / `large` (по умолчанию). Задаёт H2/H3 flow-control windows, лимиты стримов, таймауты сессий/NAT и глобальные cap'ы UDP/XHTTP relay-тасков и сессий |
...
| `tuning.udp_nat_max_entries` | Глобальный cap на число живых UDP NAT-записей (по одной на уникальный `(user, fwmark, target)`; по умолчанию зависит от профиля, `65536` для `large`). Каждая запись удерживает upstream-сокет и reader-таск до вытеснения по idle, поэтому это ограничивает footprint по файловым дескрипторам/таскам, который аутентифицированный клиент может навязать, расходясь веером по множеству адресов. Датаграммы к новым адресам отбрасываются (и считаются в `outline_ss_udp_nat_capacity_dropped_total`) при достижении лимита; живые записи этим путём не вытесняются. `0` отключает cap |
| `tuning.udp_nat_max_entries_per_user` | Per-user cap на число живых UDP NAT-записей, применяется поверх `udp_nat_max_entries` (по умолчанию зависит от профиля — `1024`/`2048`/`4096` для `small`/`medium`/`large`, доля от глобального cap'а этого профиля). Без него один аутентифицированный пользователь, расходящийся веером по множеству адресов, может занять всю таблицу и не дать остальным арендаторам открывать новые адреса до тех пор, пока не сработает idle-эвикция. Датаграммы к новым адресам пользователя, уже выбравшего свою долю, отбрасываются (тоже считаются в `outline_ss_udp_nat_capacity_dropped_total`); его живые записи продолжают работать, а слот освобождается сразу после idle-эвикции одной из них. Дефолт с запасом относительно числа одновременных адресов у легитимного клиента; `0` отключает cap и возвращает поведение «только глобальный cap» |
...
| `tuning.udp_replay_max_sessions` | Независимо ограничивает две anti-replay карты: SS-2022 session-окна и legacy salt-записи (по умолчанию зависит от профиля; `262144` для `large`). Новый `client_session_id` (SS-2022) или соль (legacy) отбрасывается при достижении лимита своей картой — это ограничивает память от клиента, который их вращает, раздувая стор. `0` отключает оба cap, а с ними и legacy salt-dedup фильтр |
| `tuning.udp_replay_max_sessions_per_user` | Per-user cap на число живых записей replay-стора (SS-2022 окна + legacy соли суммарно), применяется поверх `udp_replay_max_sessions` (по умолчанию зависит от профиля — `4096`/`8192`/`16384` для `small`/`medium`/`large`, доля от глобального cap'а профиля). Без него один аутентифицированный пользователь, распыляя уникальные session id / соли, может сам заполнить глобальный cap — и тогда стор начинает отбрасывать сессии *других* арендаторов, то есть cross-tenant DoS по доступности. С ним пользователь, выбравший свою долю, роняет только свои новые сессии (считаются с `reason="max_sessions_per_user"` в `outline_ss_udp_replay_store_full_dropped_total`); все остальные арендаторы продолжают работать, а слот освобождается сразу после idle-эвикции одной из его записей. Дефолт с большим запасом относительно одного-двух `client_session_id` легитимного SS-2022 клиента; учти, что legacy UDP чеканит новую соль на каждую датаграмму, поэтому для высоконагруженных *legacy* развёртываний повышай или отключай (`0`) этот cap. `0` отключает cap и возвращает поведение «только глобальный cap» |
| `tuning.tcp_handshake_replay_max_salts` | Максимум недавно виденных request-salt'ов Shadowsocks TCP-handshake, хранимых для anti-replay (по умолчанию зеркалит `udp_replay_max_sessions`; `262144` для `large`). Захваченный handshake, отправленный повторно дословно, выводит тот же session key и снова открывается под AEAD, поэтому salt запоминается, а повтор отклоняется. Записи истекают через `udp_nat_idle_timeout_secs`. При достижении лимита свежий salt не записывается, а handshake пропускается (fail-open) и считается в `outline_ss_tcp_handshake_replay_store_full_total`. `0` отключает фильтр |
| `tuning.xhttp_max_sessions` | Глобальный cap на число живых XHTTP-сессий в реестре (по умолчанию зависит от профиля; `262144` для `large`). Сессия создаётся на первом запросе со свежим корректным session id — *до* Shadowsocks/VLESS-аутентификации внутри relay, — поэтому это ограничивает pre-auth footprint по сессиям/буферам, который атакующий с валидным base path может навязать, распыляя уникальные id. При переполнении запросы, создающие **новую** сессию, получают HTTP 503 (считается в `outline_ss_xhttp_sessions_rejected_total{reason="max_sessions"}`); запросы к уже живому id (resume / повтор) обслуживаются всегда. `0` отключает cap |
| `tuning.xhttp_max_sessions_per_ip` | Per-source-IP cap на число живых XHTTP-сессий, применяется поверх `xhttp_max_sessions` (по умолчанию `0` на всех профилях — выключен). Без него один пир может распылять уникальные session id и в одиночку выбрать весь глобальный бюджет — ещё до того, как relay аутентифицирует клиента. Когда источник выбрал свою долю, запросы, создающие **новую** сессию, получают HTTP 503 (считается в `outline_ss_xhttp_sessions_rejected_total{reason="max_sessions_per_ip"}`); уже живой id обслуживается всегда. Ключ — **транспортный** peer-адрес, а за CDN или reverse-proxy это edge-IP, общий для всех фронтящихся клиентов, — поэтому на CDN-фронтящихся развёртываниях оставляй `0`, а включай только там, где клиенты подключаются к серверу напрямую. `0` отключает cap |
| `tuning.xhttp_max_concurrent_relay_tasks` | Глобальный cap на число одновременных XHTTP-relay-тасков по всем сессиям (зеркалит `udp_max_concurrent_relay_tasks`; по умолчанию зависит от профиля, `4096` для `large`). При достижении лимита запрос, создающий сессию, получает HTTP 503 и таск не спавнится (считается в `outline_ss_xhttp_sessions_rejected_total{reason="max_relay_tasks"}`). `0` отключает cap |
...
| `dashboard.allowed_hosts` | Имена хостов, которые origin-guard принимает вдобавок к встроенному набору (loopback-имена/адреса и адрес привязки). Нужны только за reverse-proxy, отдающим панель под DNS-именем; сравниваются по имени хоста без учёта регистра, порт игнорируется. По умолчанию пусто |
...
`[users.aliases]` разделяет один ключ на несколько **учётных** идентичностей по
source IP клиента. После обычной аутентификации (по ключу Shadowsocks или VLESS
UUID) сервер выбирает эффективный label — совпавший alias, если source IP попал в
одну из перечисленных подсетей, иначе базовый `id` — и использует его для
per-user метрик, ключа UDP NAT и логов. Аутентификацию, ключ и контроль доступа
это не меняет.
...
- **Форма значения:** одна строка CIDR/IP или список. Голый адрес — host-маршрут (`/32` или `/128`). Пересекающиеся подсети допустимы; выигрывает наиболее специфичная (longest-prefix).
- **Уникальность:** имена alias не должны совпадать ни с одним `id` пользователя или другим alias — каждый становится отдельным time series `user="…"`, поэтому коллизия молча смешала бы учёт. Конфиг, нарушающий это, отвергается на старте и в control-плоскости.
- **Кардинальность:** каждый alias добавляет одно значение label по per-user семействам метрик; ограничено числом настроенных alias.
- **Покрытие:** работает там, где сервер видит реальный IP клиента — SS-over-WS/XHTTP TCP и все несущие VLESS (WS/XHTTP/H3). SS-UDP-over-WebSocket откатывается к базовому `id` (на relay-слое peer недоступен).
- **Оговорка про CDN:** source IP — это напрямую подключённый peer. За CDN или L4-прокси это IP CDN, а не клиента, поэтому алиасинг осмыслен только на прямых подключениях — входящий PROXY-protocol / `X-Forwarded-For` сервер не читает.
...
Доступ к этому listener равносилен владению control-токенами всех управляемых инстансов, поэтому одного токена мало: токен решает, *кто* управляет панелью, но браузер сам переотправляет закешированные Basic-креды на cross-site запрос, а привязка к loopback не спасает от страницы, запущенной на машине оператора. Поэтому дашборд проверяет каждый запрос **до** разбора маршрута — так новый роут не окажется мимо проверок — и независимо от того, задан ли токен:

| Проверка | На что распространяется | При отказе |
| --- | --- | --- |
| `Host` называет этот listener | все запросы | `403` |
...
- **`Host`** принимается для loopback-имён и адресов (`localhost`, `127.0.0.1`, `[::1]`), для адреса привязки панели (при wildcard-привязке вроде `0.0.0.0` — любой литеральный адрес) и для любого имени из `allowed_hosts`. Именно это закрывает DNS rebinding: домен атакующего может резолвиться в `127.0.0.1`, но в `Host` браузер всё равно подставит *имя* атакующего. **Порт не проверяется** — доступ к loopback-панели через `ssh -L 8888:127.0.0.1:7002` или проброс порта контейнера штатен, а защиты порт всё равно не даёт: rebinding-атака обязана бить в реальный порт панели.
- **`Origin`** должен совпадать с `Host` дословно — так и происходит у same-origin запроса браузера при любом пробросе портов — либо называть хост из `allowed_hosts` (случай reverse-proxy, переписывающего `Host`). Другой локальный порт (`http://127.0.0.1:3000`) — это другой origin, он отбивается. *Отсутствие* `Origin` разрешено: `curl` и прочие небраузерные клиенты его не шлют никогда, а веб-страница подавить его не может, так что требование ломало бы скрипты операторов, ничего не закрывая.
- **`allowed_hosts`** дополняет, а не заменяет: встроенный набор остаётся в силе, поэтому loopback-установка продолжает работать и через `ssh -L`. Записи сравниваются по имени хоста без учёта регистра; порт, если он записан, игнорируется.

Для штатного UI не меняется ничего (он уже шлёт `application/json`), для `curl` — тоже. Установке, отдающей панель под DNS-именем, нужно прописать это имя в `dashboard.allowed_hosts`.
...
Изменения персистентны: каждое записывается обратно в конфиг-файл, с которым запущен сервер (`--config` / `OUTLINE_SS_CONFIG`), и только потом применяется на плоскости данных — то есть `201`/`200` означает, что изменение переживёт рестарт. Файл *патчится*, а не пересобирается: трогается ровно одна запись `[[users]]`, названная в запросе, а все остальные пользователи, секции, порядок ключей и комментарии остаются байт-в-байт прежними. Запись атомарна (временный файл + `rename`, режим и владелец цели переносятся), поэтому падение посреди записи оставляет прежний конфиг целым; если запись не удалась, вместе с ней падает и сама мутация — в рантайм ничего не уезжает.

Если сервер запущен без конфиг-файла (пользователи заданы через `--user` / `OUTLINE_SS_USERS`), патчить нечего: изменения действуют только в текущем процессе и теряются при рестарте. На старте в этом случае пишется предупреждение.
...
### 5. SNI-роутинг для чужих TLS-доменов (L4-маскировка)
...
**Наблюдаемость неудачных peek'ов.** Поток, из которого так и не удалось получить ClientHello, учитывается в `outline_ss_sni_peek_failed_total{reason=...}` по пяти корзинам:

| `reason` | Что это | Уровень лога |
| --- | --- | --- |
| `peer_closed` | Пир ушёл (FIN/EOF/RST), не отправив ClientHello | `debug` |
| `read_failed` | Чтение из inbound-сокета упало по другой причине | `warn` |
| `oversized` | Байты шли, но ClientHello не сложился в пределах `max_client_hello_bytes` | `warn` |
| `malformed` | rustls отверг байты как TLS-хендшейк | `warn` |
| `timeout` | Полный ClientHello не пришёл в пределах pre-auth-бюджета хендшейка (см. ниже) | `debug` |

`peer_closed` — это фоновый шум любого публичного порта: TCP-пробы живости, сканеры, передумавшие клиенты. Листенер видит его непрерывным потоком, и сделать с ним оператору нечего, поэтому он намеренно не пишется на `warn`, а наблюдается через счётчик. Отслеживать стоит рост `malformed`: значит, в листенер говорят чем-то почти-но-не-TLS.

**Pre-auth-таймаут хендшейка.** Каждое принятое TLS-соединение держит один из `TLS_MAX_CONCURRENT_CONNECTIONS` (4 096) permit'ов конкурентности с момента accept'а — ещё до того, как оно докажет, что это настоящий TLS-клиент. Чтобы пир, который подключился и молчит (или капает по байту — классический slowloris), не удерживал permit бесконечно, вся неаутентифицированная предхендшейковая фаза (опциональный SNI-peek *плюс* сам rustls-хендшейк) разделяет единый дедлайн ~10 с. По его истечении соединение сбрасывается, permit освобождается; таймаут учитывается как `sni_peek_failed{reason="timeout"}`, если сработал на peek'е, и как `outline_ss_tls_handshake_failed_total{reason="timeout"}`, если на хендшейке. Без этой границы 4 096 молчащих соединений исчерпали бы семафор и застопорили accept-цикл — бесплатный, полностью pre-auth отказ в обслуживании TLS-входа.
...
### 6. Несколько сертификатов на одном листенере (выбор по SNI)
...
- UDP-трафик Shadowsocks-2022 защищён скользящим anti-replay фильтром по per-session ID; дубликаты `packet_id` отбрасываются и считаются в `outline_ss_udp_replay_dropped_total`. Стор хранит не более `tuning.udp_replay_max_sessions` сессий — пакеты с новым session ID сверх этого cap отбрасываются и считаются в `outline_ss_udp_replay_store_full_dropped_total`, ограничивая память против клиента, вращающего session ID. Поверх этого глобального cap'а `tuning.udp_replay_max_sessions_per_user` ограничивает, сколько записей может держать один аутентифицированный пользователь, чтобы один арендатор не заполнил стор и не вытеснил сессии остальных; у дропов есть label `reason` (`max_sessions` — глобальный cap, `max_sessions_per_user` — арендатор, выбравший свою долю).
- TCP-handshake Shadowsocks защищён anti-replay фильтром по request-salt. Захваченный handshake (`salt || AEAD(header) || …`), отправленный повторно дословно, выводит тот же session key и снова открывается под AEAD — для SS-2022 окно таймстампа ±30 с всё ещё проходит, а у legacy AEAD таймстампа нет вовсе, — поэтому одной аутентификации мало, чтобы не переисполнить повторённый запрос. Сервер запоминает request-salt'ы поверх всех носителей (WebSocket / HTTP-3 / XHTTP) и отклоняет повтор до дозвона до upstream, считая это в `outline_ss_tcp_handshake_replay_dropped_total`. Стор хранит не более `tuning.tcp_handshake_replay_max_salts` salt'ов с TTL `udp_nat_idle_timeout_secs`; при переполнении он fail-open (пропускает handshake) и считает `outline_ss_tcp_handshake_replay_store_full_total`. Это важнее всего на plaintext SS-ingress; за TLS-носителем внешний слой уже де-дублирует, так что здесь это defense-in-depth.
- Legacy (до-2022) UDP не несёт ни счётчика пакетов, ни таймстампа, поэтому дедуплицируется по соли запроса во второй ограниченной карте с тем же cap, вытеснением и счётчиками. Так как таймстампа для ограничения срока хранения нет, защита — это окно, ограниченное памятью, а не полная: после вытеснения соли та же датаграмма снова читается как свежая. Полная replay-защита требует шифра `2022-*`; переводите UDP-пользователей на SS-2022 там, где важна устойчивость к повторам.
