...
жёстким/health, группа не кластерная, mid-session retry выключен (нет
replay-ринга), новый активный аплинк не WS-семейства, либо resume-редайл
провалился. Гейт — это исключительно опубликованный бит `soft`
(`try_soft_switch_migrate` в `pinned_relay.rs` читает `snapshot.soft`),
а не то, какой механизм вызвал переключение: автоматические
soft-switch'и — carrier-degraded автоматический failover и плановый
или ручной перевыбор (см. «Плановый перевыбор» ниже) — мигрируют живые
сессии точно так же, как явный операторский soft-switch. Только
*жёсткое* переключение (обычный health/probe-driven failover либо
любой soft-запрос, сведённый к жёсткому вне кластера) рвёт сессии.
...
- **SS-UDP** прогоняет все назначения через один групповой id (`<group>#udp` в
  `global_resume_cache`). Новый edge декодирует home-shard из id и релеит
  датаграммный носитель на home, который переприкрепляет припаркованную
  NAT-запись — один upstream source port переживает переключение.
- **VLESS-UDP** разворачивает один uplink в множество одно-назначенческих
  сессий, поэтому хранит durable per-target id в отдельном кэше
  (`global_vless_udp_resume_cache`, ключ `<group>#<target>`). Каждый target
  мигрирует независимо: свежий mux, который менеджер строит для нового edge,
  повторно предъявляет shard-несущий id этого target'а, и home ресумит
  припаркованный per-target сокет. На home VLESS-UDP едет по VLESS-TCP
...
**UDP** по-прежнему пользуется process-wide `global_resume_cache` с
ключом `<resume-scope>#udp` (плюс `global_vless_udp_resume_cache` с
ключом `<resume-scope>#<target>` для VLESS-UDP): UDP-сессия — это одна
долгоживущая сущность на аплинк (или на target), поэтому один слот
действительно именует ровно одну сессию. В конфигурациях, где UDP едет по
TCP-carrier'у (VLESS/WS, VLESS/XHTTP), UDP-слота нет вовсе — UDP следует
