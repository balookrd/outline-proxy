...
TCP- и UDP-плоскости сейчас на одном wire — сравните
`outline_ws_uplink_active_wire_index` для `transport="tcp"` и
`transport="udp"` этого аплинка; совпадающие индексы означают общий
носитель, разные — действительно независимые измерения.
