...
| `loss_sample_interval_secs`          | `10`               | с     | сетка сэмплирования счётчиков carrier-loss, независимая от `probe.interval`                       |
...
| `Failover` | failover по пробе/runtime, auto-failback, carrier-degraded failover, начальный выбор | мигрируют на кластере |
...
- `loss_sample_interval_secs` (дефолт `10`) — сетка сэмплирования.
...
Есть три разные причины, по которым у wire в конкретный момент может не
...
Оба отсутствия — структурное свойство этих путей, а не повод искать
неисправность. Не читайте «серии нет» на `xhttp_h1` или VLESS-UDP wire
как «потерь нет» — для него просто не публикуется никакого вердикта.
...
латентность аплинка в абсурдное значение. Эта фича **никогда не
исключает** лоссовый аплинк из кандидатов — только опускает его в
ранжировании. Он может быть единственным живым путём в группе, и
жёсткое исключение вывело бы его из ротации целиком вместо того, чтобы
продолжать нести трафик с пониженным приоритетом.
