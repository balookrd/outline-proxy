...
| `health_weight_floor`                | `0.05`             | [0,1] | минимальный вес выбора при `health_weighted_selection`, чтобы у постоянно падающей ноги / носителя сохранялась небольшая вероятность ретрая; анти-DPI реролл `shuffle_timer` исключает wire, чей вес просел до этого floor'а, как «не живую альтернативу» (см. ниже) — `1.0` вырожденное, но допустимое значение: любой вес клэмпится к `1.0`, поэтому реролл вместо этого считает кандидатом каждый неактивный wire |
...
| `rtt_ewma_halflife_secs`             | `300`              | с     | период полураспада влияния замера RTT на ранжирование: слот RTT конкретной ноги сохраняет вес `0.5^(возраст / период)`, затухая к следующему замеру в цепочке, и через четыре периода перестаёт считаться измеренным вовсе. Он же взводит refresh-пробу по fallback-ноге, когда слоту активной ноги исполняется один период. `0` отключает и то, и другое. См. «Старение замеров RTT» ниже |
...
| `loss_sample_interval_secs`          | `30`               | с     | сетка сэмплирования счётчиков carrier-loss, независимая от `probe.interval`; `0` полностью отключает сэмплирование (носители по-прежнему регистрируют пробы, но их никогда не дифференцируют) |
| `loss_sample_min_packets`            | `50`               | int   | минимум пакетов, которые wire должен отправить за одно окно сэмплирования, чтобы окно засчиталось |
...
| `reselect_sync`                      | `false`            | bool  | выводить порядок планового перевыбора детерминированно из имени группы, имён аплинков, локального дня и индекса слота — чтобы независимые узлы с одинаковым конфигом группы выбирали один и тот же аплинк без всякой координации. Требует `reselect_at`. См. «Синхронный перевыбор» ниже |
...
**Синхронный перевыбор (`reselect_sync`).**
...
Два узла, обслуживающих одних и тех же пользователей (например, пара клонов за
одним именем), по умолчанию ротируются независимо: каждый берёт сид розыгрыша
у ОС, поэтому уже после первого слота они сидят на разных аплинках, и клиенты
выходят с разных адресов. `reselect_sync` заменяет розыгрыш функцией от данных,
которые есть у обоих: имя группы, имена аплинков, локальный календарный день и
индекс слота хешируются (BLAKE3) в сид, а он превращается в полный порядок
предпочтения, взвешенный ТОЛЬКО сконфигурированным `weight`. Активным
становится первый элемент этого порядка, здоровый **на данном узле**.

- **Без координации.** Между узлами ничего не передаётся; смерть соседа
  никак не влияет на выбор этого узла.
- **Здоровье остаётся локальным.** Узел, у которого нога умерла, уходит с
  неё сразу и сам. Если отказ видит только один из двух — они разъедутся до
  следующего слота: внутрисуточного схождения нет.
- **Ротация сохраняется** за счёт исключения *детерминированного* победителя
  предыдущего слота (он считается так же, без фильтра по здоровью, поэтому
  все узлы исключают один и тот же аплинк). Исключение снимается, если из-за
  него не осталось бы ни одного здорового кандидата: сойтись на единственной
  живой ноге важнее.
- **Локальные штрафы намеренно не участвуют** в порядке — они у каждого узла
  свои и свели бы общий сид на нет. На обычный failover это не влияет: его
  флаг не трогает.
- **Старт следует слоту, а не сохранённому состоянию.** Перезапущенный узел
  оказывается там же, где только что запущенный, а не возвращается на ногу,
  на которой его остановили: она может быть следом одностороннего failover,
  за которым сосед не пошёл. При старте сначала прогоняется проба, затем
  применяется решение слота.
- **`POST /control/reselect` становится идемпотентным внутри слота**: он
  переприменяет решение слота (`outcome = "skipped"`, если узел уже стоит
  правильно), а не разыгрывает новое. Нажать его на обоих узлах — самый
  быстрый способ свести их, не дожидаясь следующего слота.

Требования и оговорки: `reselect_at` обязателен — `reselect_interval`
срабатывает относительно старта каждого процесса и согласоваться между узлами
не может, поэтому загрузчик отвергает такую комбинацию. Все узлы одного домена
синхронизации обязаны быть в одной таймзоне: слоты и ключ дня — локальное
время. Список аплинков входит в сид, поэтому лишняя нога на одном узле не
меняет общее решение пары, а переводит этот узел в другой домен синхронизации.
Наконец, порядок выводится из конфигурации, а не из секретов: он воспроизводим
любым, кто знает имена группы и аплинков.
...
### Старение замеров RTT

**Зачем это нужно.** Замер RTT ранжировал аплинк вечно, каким бы старым он
ни был. Для любой ноги, кроме primary, это самоподдерживающаяся ловушка:
плохой score не даёт балансировщику отправить аплинку трафик, а именно
трафик породил бы новый замер. 10 августа 2026 года узел, которому починили
MTU несколькими часами раньше, всё ещё ранжировался по сэмплу 4,18 с,
снятому в момент поломки: базовая латентность всё это время пробивалась как
0,253 с, потери носителя не измерялись вовсе, а отдельный клиент,
закреплённый ровно на том же носителе, показывал 0,199 с. Лечилось только
удалением аплинка из конфига и повторным apply — потому что именно это
создаёт пустые слоты.

**Что стареет.** EWMA RTT каждой ноги несёт момент последнего изменения. При
ранжировании слот сохраняет вес `0.5^(возраст / rtt_ewma_halflife_secs)`, а
остаток веса уходит следующему звену той же цепочки, которую ранжирование
обходит и сейчас: слот активной ноги, затем EWMA primary, затем последний
probe-сэмпл. После четырёх периодов полураспада слот перестаёт считаться
измеренным вовсе.

**Чего затухание не делает.** Оно никогда не делает аплинк лучше, чем
что-либо реально про него измеренное: устаревший слот затухает к другому
*замеру*, а не к нулю, и слот, за которым в цепочке ничего нет, сохраняет
полный вес независимо от возраста — выбросить его значило бы убрать аплинк
из ранжирования, а это заявление куда сильнее, чем «число старое». Только
что отказавший носитель по-прежнему прижат механизмами, которые по этой
кривой не затухают вовсе: per-wire штрафом за живучесть
(`health_weighted_selection`) и надбавкой к штрафу от окна carrier-descent.
Свежие плохие сэмплы, как и раньше, входят немедленно и с полным весом —
через EWMA.

**Refresh-проба.** Одно только затухание решало бы по предположению, а не по
измерению, поэтому, как только слоту активной ноги исполняется один период
полураспада, проба по fallback-ноге (раньше она запускалась только при
провале primary-пробы) отрабатывает и на этом цикле. Это не более одного
лишнего хендшейка на мультиwire-аплинк за период полураспада; частота
задаётся `rtt_ewma_halflife_secs`, а не `probe.interval`, поэтому уплотнение
расписания проб их не размножает. Аплинки без fallback'ов за это не платят
вовсе — слот primary освежает обычная проба на каждом цикле.

**Как читать в метриках.**

| серия | смысл |
| ----- | ----- |
| `outline_ws_uplink_active_wire_rtt_ewma_seconds` | что измерено на ноге, несущей трафик; отсутствует, когда слот истёк |
| `outline_ws_uplink_active_wire_rtt_age_seconds`  | сколько этому замеру лет |
| `outline_ws_uplink_latency_inflated_seconds`     | по чему ранжирование действительно решает — после инфляции от потерь и взвешивания по возрасту |

Расхождение первой строки с третьей имеет две причины, и различает их
возраст: большой возраст означает, что ранжирование почти перестало верить
слоту, малый — что свежий замер раздувают потери носителя.
`rtt_ewma_halflife_secs = 0` отключает и затухание, и refresh-пробу, возвращая
поведение до 1.6, где первое звено с любым значением выигрывает безоговорочно.
...
**Это измерение в направлении отправки.** И QUIC-`lost_packets`, и
TCP-`tcpi_total_retrans` считают пакеты, которые **отправил именно этот
узел** и вынужден был признать потерянными или ретранслировать — никогда
пакеты, которые пир не смог доставить нам. Путь, лоссовый преимущественно
в направлении **сервер→клиент** (downlink — инцидент 2026-08-02 был про
downlink-видео), попадает в этот сигнал только косвенно, через
задержанные или потерянные ACK, раздувающие счётчик ретрансляций самого
клиента, а не как прямое измерение downlink-потерь. Держите это в уме,
калибруя `loss_latency_penalty_k` или `loss_failover_ratio` под симптом,
проявляющийся на downlink: число, которое вы настраиваете, — это «сколько
переотправил этот узел», а не «сколько потерял другой».
...
- `loss_sample_interval_secs` (дефолт `30`) — сетка сэмплирования. `0` —
  ...
  фичи. Не совсем бесплатно: вытеснение мёртвых/простаивающих проб
  происходит только внутри тика сэмплирования, поэтому с выключенным
  таймером каждая пара (аплинк, transport, wire) может накопить до
  `MAX_PROBES_PER_WIRE` (8) дублированных дескрипторов, прежде чем
  собственный «новейший побеждает» лимит registry начнёт вытеснять
  самые старые на следующем дозвоне. Ограничено и самоограничивается,
  не утечка — но и не полностью бесплатный no-op.
- `loss_sample_min_packets` (дефолт `50`) — минимальный объём на окно, считается в пакетах, которые **отправила** эта сторона.
...
**Hot-swap через `/control/apply` сбрасывает сигнал потерь.** Registry
carrier-loss у нового менеджера всегда стартует пустым — носители,
дозвоненные под предыдущим менеджером, остаются зарегистрированы там, а
не здесь. Такие носители не дают сигнала потерь, пока их не дозвонят
заново естественным образом (конец сессии, переключение wire, refill
standby) — самовосстанавливается за один цикл дозвона, не утечка, но
стоит иметь в виду, если отношение выглядит свежесброшенным сразу после
apply.

**Как читать метрики.** Три gauge, все с лейблами `{group, transport,
uplink}`:

- `outline_ws_uplink_carrier_loss_ratio` — сглаженное отношение потерь
  (EWMA, `loss_ewma_alpha`) для wire, несущего трафик сейчас.
- `outline_ws_uplink_carrier_loss_observed_packets` — пакеты, на
  которых основано это отношение, кумулятивно **с последнего сброса
  wire** (свежий дозвон либо потеря wire последнего зарегистрированного
  носителя — см. причину 4 ниже), а не с запуска процесса. Он говорит
  *как долго измеряется этот wire*, а не *насколько можно доверять
  текущему значению отношения*: отношение — это EWMA с фиксированной
  памятью примерно на последние десять засчитавшихся окон
  (`loss_ewma_alpha = 0.2`), поэтому wire с миллионом пакетов,
  накопленным за неделю, скорится по тому же объёму недавней истории,
  что и wire с тысячей пакетов за последний час. Используйте этот
  счётчик, чтобы отличить «трафика пока мало» от «измерено» (причина 1
  ниже), а не чтобы судить о том, насколько доверять текущему значению
  отношения.
- `outline_ws_uplink_latency_inflated_seconds` — латентность, по которой
  выбор реально ранжирует после свёртки carrier-loss
  (`base_latency × (1 + loss_latency_penalty_k × loss)`, зажатая
  `loss_latency_inflation_max`), рядом с уже существующим
  `outline_ws_uplink_rtt_ewma_seconds`. При дефолте поставки
  (`loss_latency_penalty_k = 0.0`) он равен обычному RTT EWMA; как
  только `k` задан, сравнение этих двух серий и показывает реально
  применяемый штраф.

**Когда обе плоскости сидят на одном wire, `tcp` и `udp` — это одно и
то же измерение дважды, а не два независимых.** `CarrierLossRegistry::register`
(`crates/outline-uplink/src/loss.rs`) дедуплицирует пробы по ключу
`(transport, wire, identity)` — `transport` здесь часть ключа, а не
фильтр того, что измеряется. На носителе, который реально общий для
обеих плоскостей (носители `xhttp_h3` парка: одно QUIC-соединение
обслуживает все сессии, и TCP, и UDP), один и тот же `quinn::Connection`
регистрируется дважды — один раз под `transport = "tcp"`, другой раз под
`transport = "udp"`, и каждая регистрация читает весь `PathStats` этого
соединения. Ни одна из серий сама по себе не ошибочна; пара — это один
носитель, посчитанный дважды под двумя лейблами. Применимо ли это к
конкретному аплинку в данный момент, зависит от того, сидят ли его
TCP- и UDP-плоскости на одном *носителе* — а это не тот же вопрос, что
«на одном wire». Совпадение индекса wire необходимо для дублирования,
но не достаточно: VLESS строит свой UDP-мукс на каждое назначение,
отдельно от TCP-соединений, поэтому обе плоскости могут показывать
`active_wire_index = 0` и при этом измерять два разных QUIC-соединения.
Замер на `senko`, когда обе плоскости были на wire 0: `tcp` показывал
19.58% потерь на 49 142 пакетах, а `udp` — 0.0038% на 144 167. Разница
в четыре порядка при одном и том же индексе wire. Надёжный признак —
сами значения: при дублировании обе серии совпадают пакет в пакет,
поэтому равные числа означают один носитель, а расходящиеся — два,
что бы ни показывали индексы.

Замер на парке: пакеты, засэмплированные за 2-минутный интервал на
аплинке `senko` (`wire tcp`/`wire udp` — это `active_wire_index` по
каждой плоскости):
...
Корреляция точная: одинаковый индекс wire на обеих плоскостях — обе
серии описывают один и тот же носитель, пакет в пакет идентичны; разные
индексы wire — два разных носителя, и серии расходятся. Именно так
дублирование впервые и заметили — оператор наблюдал, как строка потерь
с лейблом `tcp` растёт синхронно с QUIC-видеотрафиком пользователя,
потому что эта строка на самом деле описывала то самое QUIC-соединение,
которое несло видео. Registry, sampler и сами метрики намеренно
оставлены как есть: аплинк, у которого обе плоскости сидят на одном
wire, — это реальная и распространённая конфигурация (дефолт для
парка), а дедупликация пары потребовала бы атрибутировать потери
«носителю», а не «плоскости» — изменение куда крупнее, чем оправдывает
эта оговорка. Читайте строку `tcp` на аплинке с общим wire как
описывающую носитель целиком, включая любой `udp`-трафик, который по
нему едет, — а не как изолированное TCP-only измерение.

**Предел этой оговорки: на разных wire обе плоскости — полностью
независимые измерения.** Замер несколькими минутами позже на том же
аплинке, с `tcp` на wire 0 и `udp` на wire 3:
...
Это узел, стримящий видео по QUIC: UDP-плоскость закономерно несёт
объём примерно в четыре раза больше, чем TCP-плоскость, и оба числа
потерь действительно разные, потому что происходят от двух разных
носителей. Правило для чтения что дашборда, что `/metrics` напрямую:
сравнивайте `outline_ws_uplink_active_wire_index` обеих плоскостей,
прежде чем сравнивать их числа потерь. Совпадающий индекс wire значит
«один и тот же носитель, не считать его за две точки данных»; разный
индекс — «оба числа заслуживают доверия по отдельности».
...
1. Он ещё не отправил `loss_sample_min_packets` (дефолт `50`) пакетов
...
`loss_sample_interval_secs` на дефолте (`30`) или любом другом ненулевом
значении.
...
- сам находится на уровне `loss_failover_ratio` или ниже, **на свежем
  измерении** — та же проверка «последнее засчитавшееся измерение
  укладывается примерно в 3 тика сэмплирования», что описана выше для
  активного, применённая к собственному отношению кандидата. Кандидат
  **без** собственного вердикта по потерям считается чистым:
  ...
  лоссовом пути без всякой причины. Кандидат, у которого вердикт
  *есть*, но он устарел — standby wire, который пингуют слишком легко,
  чтобы он хоть раз снова набрал `loss_sample_min_packets`, — **не**
  считается чистым просто потому, что застывшее число читается низким:
  измерение, которое никто не подтвердил заново, не доказывает, что
  кандидат чист *сейчас*, только что он был таким когда-то.
...
- `outline_ws_uplink_loss_failovers_total{transport, group, from_uplink,
  ...
  `transport` — это transport, чьё отношение потерь фактически
  обусловило решение, а не обязательно transport dispatch'а, вызвавшего
  переоценку — при `routing_scope = "global"` гейт всегда TCP, поэтому
  UDP-dispatch, вскрывший переключение, вызванное потерями на TCP,
  считается под `transport="tcp"`, а не `"udp"`.
...
#### Какой ingress проходит цепочку wire'ов

Всё, что описано ниже — dial-loop, sticky-fallback state-машина,
`shuffle_wires`, per-wire отслеживание RTT / carrier-loss — это один
механизм (`dial_over_wires`), общий для обоих ingress'ов. Эта общность
появилась недавно. Пока не было `[load_balancing] tun_wire_dial`, эта
цепочка была доступна только из SOCKS ingress: TUN-флоу всегда дозванивался
на primary-носитель аплинка (wire 0) — и точка. Поэтому сломанный primary
стоил TUN'у *всего аплинка целиком* — state-машина ниже существовала и
продвигала `active_wire`, но TUN ничего из неё не читал перед дозвоном, так
что конфиг с тремя fallback'ами и `shuffle_wires = true` выглядел как
настроенная избыточность, а по факту каждый TUN-дозвон уходил на wire 0.

При `tun_wire_dial = true` TUN ingress вызывает тот же самый
`dial_over_wires`, что и SOCKS ingress всегда вызывал, на обеих плоскостях —
TCP и UDP: свежий TUN-дозвон проходит настроенную fallback-цепочку,
warm-standby пул прогревается на текущем активном wire'е аплинка, а живой
TCP-флоу, у которого умер носитель, мигрирует на этот активный wire вместо
повторного дозвона на wire 0. UDP-миграция живого флоу переносит состояние
только между wire'ами *одного и того же* прокси-протокола (SS↔SS,
VLESS↔VLESS) — серверный SS-UDP datagram-park и VLESS-UDP mux-park — это
разные механизмы без передачи состояния между ними, так что смена протокола
на уровне wire'а посреди флоу начинает новую UDP-сессию вместо возобновления
старой.

Флаг задаётся на группу (`[[uplink_group]] tun_wire_dial = true`, либо
`[outline.load_balancing]` в inline-форме одного аплинка) и по умолчанию
`false` — раскатывайте по одному узлу за раз. При выключенном гейте
поведение побайтово совпадает с до-`tun_wire_dial`: каждый TUN-дозвон
просит wire 0 и никогда не трогает state-машину wire'ов, так что по узлу с
выключенным гейтом нельзя понять, что фича вообще раскатана.
...
Каждый тик, независимо для TCP и UDP:

* Выбирает случайный wire среди всех, кроме **того, что сейчас
  активен** на этом транспорте — так реролл всегда реально меняет
  wire, а не «рероллит» на тот же самый по случайности. Wire, чей
  вес живучести (тот же затухающий штраф, которым
  `health_weighted_selection` ранжирует wires) просел до
  `health_weight_floor`, тоже исключается — наравне с активным
  wire: это не *живая* альтернатива. При `health_weighted_selection
  = true` выбор среди оставшихся кандидатов смещён в сторону более
  здоровых (та же взвешенность, что и везде); при `= false` выбор
  среди того же набора кандидатов равномерный — но исключение в
  обоих случаях одинаковое, так что «меняет ли реролл wire» никогда
  не зависит от этого флага.
* **Если у транспорта нет живой альтернативы** (все остальные wires
  сейчас на floor), активный wire этого транспорта на этом тике
  остаётся **полностью нетронутым**: без обновления pin, без сброса
  failure-счётчиков — ничего не применяется. Журнал явно логирует
  этот случай (`"no live TCP/UDP alternative ... active wire left
  unchanged"`), чтобы это читалось как «не нашли, куда рероллить», а
  не как молчаливое бездействие. TCP и UDP независимы: отсутствие
  живой альтернативы у одного транспорта никогда не блокирует
  реролл другого.
* Когда реролл всё-таки приземляется на новый wire, он обнуляет
...
Wire, исключённый из розыгрыша на одном тике, не становится
недостижимым навсегда: его штраф затухает по обычной кривой
`failure_penalty_halflife`, доказанная сквозная доставка по нему
обнуляет штраф целиком, а `wire_dial_order` (по которому идёт
dial-каскад внутри сессии) по-прежнему возвращает полную
перестановку, включающую просевшие до floor'а wires — так что такой
wire продолжает нести трафик внутри сессии и может снова стать
`active_wire` через обычный dial/probe путь, независимо от
`shuffle_timer`. Исключение — это per-tick фильтр живучести, а не
постоянный запрет «никогда не wire X»: он самовосстанавливается, как
только wire снова докажет себя.
...
отсутствием живой альтернативы, метрику этого транспорта не пишет.
