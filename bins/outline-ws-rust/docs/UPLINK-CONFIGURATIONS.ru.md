...
Описывает пять поддерживаемых форм блока `[[outline.uplinks]]`: на
...
- **TCP fallback-цепочка:** `xhttp_h3 → xhttp_h2 → xhttp_h1`, идентично
  спуску VLESS-XHTTP (см. раздел 6). На h1-носителе stream-one
...
share-link (раздел 7). Вместо ручного заполнения полей `ss_*_url` / `ss_mode` /
...
- `type=quic` для share-link отвергается — raw QUIC нативно мультиплексирует
  TCP + UDP и не имеет combined-path формы; для raw QUIC используй длинную
  форму `transport=ss`.
...
Линк `type=quic` кладёт URL в `vless_ws_url` с `vless_mode = "quic"` (важны
только host + port; путь для raw QUIC игнорируется). В отличие от SS, VLESS
мультиплексирует TCP + UDP на **одном** пути, поэтому dial-URL всегда один вне
зависимости от носителя — кодировать split / combined не нужно.
...
«Share-link URI (`ss://`)» в разделе 3. Схема выбирает транспорт.
...
- Тот же eligibility-gate, что у v1 — SS-WS / VLESS-WS /
  VLESS-XHTTP carriers; raw QUIC вне scope'а (нет HTTP-layer
  carrier'а для v2-negotiation).
...
- **Вертикальный carrier-каскад до wire-rotation**: для WS-family
  ...
  **effective mode активного wire** — пока активный wire не на дне
  ...
  (или family без descent stack: raw QUIC ALPN cases) —
...
- Fallback TCP- и UDP-дайлы участвуют в
  ...
  header (WS, VLESS-WS, VLESS-XHTTP). У raw-QUIC fallback-wire'а нет
