...
уровне порога или ниже полностью сбрасывает эпизод. Отношению доверяют,
только пока оно *свежее* — последнее *засчитавшееся* измерение wire
(набравшее `loss_sample_min_packets`) должно укладываться примерно в
3 тика сэмплирования. Без этой проверки свежести wire, который один раз
измерил высокие потери, а затем несёт только лёгкий трафик ниже
объёмного порога (редкие keepalive, ночное затишье), продолжал бы читать
своё последнее, уже неактуальное отношение как непрерывное доказательство
бесконечно. Эпизод «старел» бы на измерении, которое давно перестали
наблюдать, и реально настроенная оператором длительность потерь
...
набран) сбрасывает эпизод так же, как и чистый тик. Как только этот
эпизод непрерывно продержался не меньше `loss_failover_secs`, активный
уступает кандидату, который:
