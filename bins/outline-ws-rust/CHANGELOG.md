...
- **Adaptive carrier padding for the WS / XHTTP dials (anti TLS-in-TLS record-size correlation).** Each Shadowsocks chunk is wrapped in a length-delimited padding frame so the outer TLS record size no longer tracks the payload size. Global on the client (every WS / XHTTP dial is padded when enabled), config-synchronised with the server, off by default; optional idle cover traffic on the uplink. Covers SS-over-WS (h1 / h2 / h3) and SS-over-XHTTP; UDP and raw-QUIC are out of scope.
