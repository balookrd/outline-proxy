...
- `endpoint_check` (по умолчанию `false`): предварительная проверка достижимости голым TCP-connect по всем эндпоинтам аплинка (primary + fallback'и, обе плоскости, дедуп по `host:port` плюс параметры егресса), выполняется параллельно перед carrier/application-пробами. После `min_failures` циклов, в которых **ни один** эндпоинт не принял connect, аплинк сразу помечается нездоровым — без carrier-спуска и без обхода wire за wire — а список эндпоинтов попадает в `last_error`. Именно это превращает «хост выключили» из многоминутного открытия (`h3 → h2 → h1` на каждом wire, каждый шаг платит полный `timeout`) в пару циклов. Достижимый эндпоинт не утверждает ничего: здоровье по-прежнему определяет обычная проба. Держите выключенным, если эндпоинт отдаёт QUIC/H3 по UDP при закрытом TCP-порте — проверка прочтёт это как «сервер мёртв».
