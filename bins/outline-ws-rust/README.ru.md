...
- `endpoint_check` (по умолчанию `false`): предварительная проверка достижимости голым TCP-connect по всем эндпоинтам аплинка (primary + fallback'и, обе плоскости, дедуп по `host:port` плюс параметры егресса), выполняется параллельно перед carrier/application-пробами. Это **ускоритель, а не судья**: обычная проба выполняется в любом случае, и последнее слово о здоровье остаётся за ней. Отказ засчитывается как хостовый, только когда обе согласны — ни один эндпоинт не принял connect **и** сама проба не прошла ни по одной плоскости; тогда после `min_failures` таких циклов аплинк помечается нездоровым без carrier-спуска (`h3 → h2 → h1`) и без обхода цепочки wire, а список эндпоинтов попадает в `last_error`. Именно это превращает «хост выключили» из многоминутного открытия в пару циклов. Прошедшая проба перебивает предпроверку и обнуляет её серию: голый connect не отличает «сервера нет» от «новый TCP к нему отклоняют, пока трафик идёт по уже поднятому QUIC-носителю», и путать эти случаи нельзя.
- `endpoint_check_timeout_ms` (по умолчанию `2000`): дедлайн на один эндпоинт для `endpoint_check`, включая резолв. Держите его заметно ниже `timeout_secs`: мёртвый хост стоит полного дедлайна каждый цикл, потому что SYN в чёрную дыру не вызывает RST. Результаты пишутся как `probe="endpoint"` в `outline_ws_probe_runs_total` / `outline_ws_probe_duration_seconds`.
