...
- `soft` учитывается только на группе с `shared_resume`. Везде остальном `/control/activate` зажимает его до hard switch и возвращает фактическое значение в ответе.
- Hard switch, health-failover и любой другой автоматический перевыбор остаются как были: RST со счётчиком `global_switch`.
- Автоматический **carrier-degraded failover** и плановый **reselect** тоже публикуют soft switch, поэтому получают такое же обращение.
