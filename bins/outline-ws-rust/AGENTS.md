...
> rustls-провайдер — `aws-lc-rs`, в графе зависимостей должен оставаться ровно
> один `CryptoProvider`. Описание «workspace» ниже относится к историческому
...
feature gates частью публичного поведения.
...
- `crates/outline-wire/`: общий с сервером wire-кодек (`CipherKind`,
  ...
  wire-форматы правятся там (это затрагивает и `outline-ss-rust`).
...
- `config.toml`: пример серверной конфигурации.
- `vendor/h3`: patched vendored dependency. При изменении vendored patches
  обновляйте корневые `PATCHES.md` / `PATCHES.ru.md` (единый источник истины).
...
Если изменение затрагивает feature gates или опциональный код
metrics/dashboard/TUN/H3, дополнительно проверьте сборку с отключёнными
дефолтными фичами:
...
- Буферы чтения в relay-циклах идут через `outline_net::RelayReadBuf`
  (`crates/outline-net/src/relay_buf.rs`), а не через `Vec::with_capacity(...)`
  на каждой итерации: одна аллокация переиспользуется, пока флоу активен, и
  освобождается, когда парковка переживает `RELAY_BUF_IDLE_GRACE` — простаивающая
  сессия по-прежнему не держит буфер. Две формы неслучайны: `fixed` для датаграмм
  (короткое окно → ядро молча УСЕКАЕТ датаграмму, поэтому окно всегда
  `MAX_RECV_UDP_DATAGRAM_SIZE` = 65_535, покрывая и IPv6-максимум 65_527) и
  `adaptive` для стримов (короткое окно стоит лишней итерации, не данных, поэтому
  старт 16 KiB с удвоением до `SHADOWSOCKS_MAX_PAYLOAD`). Не возвращай
  per-iteration 64 KiB-аллокацию и не сужай UDP-окно до IPv4-потолка 65_507:
  oversize-пакеты отбраковываются WARN'ом ПОСЛЕ чтения
  (`MAX_UDP_RELAY_PACKET_SIZE` / `MAX_CLIENT_UDP_PACKET_SIZE`), а не усечением
  внутри него.
...
- TLS cert-expiry check: проверка срока сертификатов endpoint'ов аплинков
  ...
  (проверка:
  `cargo tree -e features --no-default-features --features control | grep x509`
...
- Возраст замера RTT — часть замера. Слоты `PerTransportStatus::rtt_ewma` /
  `fallback_rtt_ewma` держат `crate::rtt::RttEwma` (значение + момент
  последнего изменения), и ранжирование в `base_latency_with` взвешивает
  каждое звено цепочки на `0.5^(age / rtt_ewma_halflife)`, отдавая остаток
  веса следующему звену (слот активного wire → EWMA primary → последний
  probe-сэмпл). Причина: слот fallback-wire никто не освежает регулярно —
  проба по нему шла только при провале primary, а реальный дайл туда
  попадает лишь после того, как балансировщик уже выбрал этот wire, поэтому
  замер, снятый на сломанном носителе, ранжировал аплинк вечно и сам себе не
  давал обновиться (2026-08-10, sebek). Инварианты, которые нельзя ломать:
  затухание ведёт к другому ИЗМЕРЕННОМУ звену, а не к нулю (иначе устаревание
  делало бы аплинк лучше, чем он измерен), звено без следующего сохраняет
  полный вес (иначе аплинк выпадает из ранжирования целиком), каждое звено
  инфлируется потерями СВОЕГО wire до блендинга (пара latency↔loss из
  `base_latency_and_wire_loss` не должна разъезжаться), а `record` при
  `sample = None` не двигает и отметку времени. Парная половина —
  `active_wire_rtt_needs_refresh` в `manager/probe/scheduler.rs`: она гоняет
  `run_fallback_wire_probe` ещё и на здоровом primary, когда слоту активного
  wire исполнился один период полураспада. Наблюдаемость: сырое значение
  (`*_active_wire_rtt_ewma_ms`, гаснет только на полном истечении), возраст
  (`*_active_wire_rtt_age_ms`) и ранжируемое значение
  (`*_inflated_latency_ms`) — три разные величины, не схлопывай их в одну.
