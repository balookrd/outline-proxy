...
- **Адаптивный carrier-padding для WS / XHTTP dial'ов (против корреляции по размеру TLS-записи при TLS-in-TLS).** Каждый Shadowsocks-чанк оборачивается в length-delimited padding-фрейм, так что размер внешней TLS-записи больше не следует за размером полезной нагрузки. Глобально на клиенте (каждый WS / XHTTP dial паддится, когда включено), синхронизируется с сервером по конфигу, по умолчанию выключено; опциональный idle cover-трафик на аплинке. Покрывает SS-over-WS (h1 / h2 / h3) и SS-over-XHTTP; UDP и raw-QUIC вне scope.
