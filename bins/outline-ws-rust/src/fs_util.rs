...
/// to, its owner: the client `config.toml` holds the SOCKS5 and per-user
/// passwords, uplink Shadowsocks passwords / cluster PSK, VLESS ids and the
/// control/dashboard tokens, so a mutation that widened its mode would expose
/// every credential to any local user, and one that changed the owner would
/// leave a config the service user can no longer load. Contents are fsynced
/// before the rename and the parent directory after it, so the swap also
/// survives a power loss rather than only a process kill.
...
/// ambient umask: it briefly holds the same passwords, PSK and tokens as the
...
/// the target *including* both, so without this an admin-set `600
/// outline-ws-rust:outline-ws-rust` would decay to the writer's identity at
/// the ambient umask (typically world-readable 0644). A missing target (first
/// write) keeps the private mode from [`write_temp`].
#[cfg(unix)]
