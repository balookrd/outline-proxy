...
use super::uplinks_crud::{handle_uplinks, handle_uplinks_reorder};
...
async fn handle_request(request: Request<Incoming>, state: Arc<ControlState>) -> ControlResponse {
    ...
    match label_path {
        ...
        "/control/uplink_enabled" => {
            let response = handle_set_enabled(request, state.uplinks.clone()).await;
            record_metrics_http_request("/control/uplink_enabled", response.status().as_u16());
            response
        },
        "/control/uplinks" => {
