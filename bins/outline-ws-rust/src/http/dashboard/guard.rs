...
//!
//! The dashboard parses request bodies by hand, so nothing forces a preflight
//! the way an `application/json` extractor would on the server plane. A
//! `Content-Type: text/plain` POST is a *simple* request in CORS terms: it is
//! sent without preflight, and `mode: "no-cors"` hides the response but not the
//! effect — the uplink still switches, the config still applies. Binding to
//! loopback does not help, because the attacker's page runs on the operator's
//! machine. And with no `Host` check, an attacker-controlled domain that
//! resolves to `127.0.0.1` (DNS rebinding) reaches the panel as if it were
//! local.
//!
//! Three checks, applied to every request before routing so a newly added
//! route cannot slip past them:
//!
...
//! panel through `ssh -L 8888:127.0.0.1:9092` or a container port mapping is
...
use super::response::{DashboardResponse, json_error};
...
impl OriginPolicy {
    ...
    pub(super) fn rejection(
        ...
        if !self.host_allowed(host) {
            ...
            return Some(json_error(StatusCode::FORBIDDEN, "unexpected Host header"));
        }
        ...
        if let Some(origin) = headers.get(ORIGIN) {
            ...
            if !self.origin_allowed(origin, host) {
                ...
                return Some(json_error(StatusCode::FORBIDDEN, "cross-origin request rejected"));
            }
        }
