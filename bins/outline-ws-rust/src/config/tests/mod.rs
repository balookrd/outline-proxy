...
#[tokio::test]
async fn load_config_parses_bypass_when_down_flag_on_groups() {
    let tmp = TempDir::new().unwrap();
    let path = tmp.path().join("config.toml");
    std::fs::write(
        &path,
    ...
    let args = super::Args::parse_from(["test"]);
    let config = super::load_config(&path, &args).await.unwrap();
    assert!(config.groups[0].load_balancing.bypass_when_down);
    // Unset groups keep the legacy park-on-down-group behaviour.
    assert!(!config.groups[1].load_balancing.bypass_when_down);
}

#[tokio::test]
async fn load_config_parses_bypass_when_down_flag_in_legacy_load_balancing() {
    let tmp = TempDir::new().unwrap();
    let path = tmp.path().join("config.toml");
    std::fs::write(
        &path,
...
async fn load_config_reads_dashboard_instances() {
    ...
    std::fs::write(
        ...
        allowed_hosts = ["panel.example.com", "  ", " proxy.example.com "]
    ...
    // Blank entries are dropped and the rest trimmed: a stray whitespace entry
    // would otherwise become a host name nothing can ever match.
    assert_eq!(dashboard.allowed_hosts, vec!["panel.example.com", "proxy.example.com"]);
    assert_eq!(dashboard.instances[0].name, "inst-a");
    assert_eq!(dashboard.instances[0].token, "dash-token");
}

/// Without the key the dashboard falls back to its built-in host set —
/// loopback plus the bound address — rather than to "anything goes".
#[cfg(feature = "dashboard")]
#[tokio::test]
async fn load_config_defaults_dashboard_allowed_hosts_to_empty() {
    let tmp = TempDir::new().unwrap();
    let dir = tmp.path();
    let path = dir.join("config.toml");
    std::fs::write(
    ...
    let args = super::Args::parse_from(["test"]);
    let config = load_config(&path, &args).await.unwrap();
    assert!(config.dashboard.unwrap().allowed_hosts.is_empty());
}
