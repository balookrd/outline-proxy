...
- `endpoint_check` (default `false`): bare-TCP reachability pre-check across every endpoint of the uplink (primary + fallbacks, both planes, deduplicated by `host:port` plus egress options), run concurrently before the carrier/application probes. It is an **accelerator, not a judge**: the regular probe runs either way and keeps the last word on health. Only when both agree — no endpoint accepts a connect *and* the probe itself got through on neither plane — is the failure treated as host-level, and then the uplink is marked unhealthy after `min_failures` such cycles without walking the carrier descent (`h3 → h2 → h1`) or rotating the wire chain, with the endpoint list surfaced in `last_error`. That is what turns "host switched off" from a multi-minute discovery into a couple of cycles. A probe that got through overrules the pre-check and resets its streak: a bare connect cannot tell "the server is gone" from "new TCP to it is refused while traffic rides an already-established QUIC carrier", and the two must not be treated alike.
