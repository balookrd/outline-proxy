...
Всё остальное выводится. `url_base` — это `public_host` плюс последний сегмент
`write_dir`, то есть те же два значения, написанные второй раз; задавать его
отдельно нужно, только если артефакты раздаются не с того хоста, куда
записаны. `public_scheme` по умолчанию `wss` (и определяет схему выведенного
URL: `wss` → `https`, `ws` → `http`). Ключ `print` генератор не читает вовсе.
...
120 тестов, stdlib-only, в сеть не ходят и за пределы временного каталога не
