...
| `<user>.json` | есть `vless_id` и VLESS-эндпоинт | Xray-подписка с балансером cloud1+cloud2 |
...
## Секция `[access_keys]` — три ключа

```toml
[access_keys]
public_host = "cloud.beerloga.su"
file_extension = ".conf"
write_dir = "/var/www/html/<keys-dir>/"
```

Всё остальное выводится. `url_base` — это `public_host` плюс путь `write_dir`
**относительно корня nginx** (`/var/www/html` на всех узлах парка), то есть те
же значения, написанные второй раз. Вложенный каталог сохраняет весь путь:
`/var/www/html/keys/prod` → `/keys/prod`, а не `/prod`.

`write_dir` **вне корня nginx** не даёт вывести ничего — URL-путь из пути в
файловой системе там неизвестен, а придумать его значит получить худший из
отказов: артефакты записаны верно, а ссылки внутри ведут в несуществующий
каталог. Такой узел обязан прописать `url_base` явно; явное значение всегда
выигрывает и нужно ещё в случае, когда артефакты раздаются не с того хоста,
куда записаны.

`public_scheme` по умолчанию `wss` и задаёт схему выведенного URL
(`wss` → `https`, `ws` → `http`). Ключ `print` генератор не читает вовсе.

### Узел без раздачи

На `.102` и `.104` nginx выключен: артефакты нужны на диске, но публиковать их
некому. Там `write_dir` указывает **вне вебрута** —
`/opt/outline/access-keys-out/` — и это не мелочь оформления, а два следствия
сразу:

- `url_base` не выводится, поэтому в `<user>.txt` не попадают `ssconf://` и
  `happ_url`, которые всё равно вели бы в никуда. Остаются рабочие `ss://` и
  `vless://` — они адресуют прокси-порт узла, а не HTTP-раздачу;
- файлы с паролями и `vless_id` не лежат в каталоге, который опубликуется сам
  собой, стоит кому-нибудь однажды поднять nginx.

То есть «раздаём / не раздаём» выражено местом хранения, а не отдельным
флагом, — и генератор приходит к правильному ответу без единого условия про
конкретные узлы.

Сервер эти ключи парсит, но не использует, — секция целиком принадлежит
генератору.
...
written_json: <путь>        # если есть vless_id и VLESS-эндпоинт
...
у VLESS-only — `outline_url` и `written_conf`.

`outline_url` — тот же файл, что `written_conf`, но со схемой `ssconf://`:
Outline ходит по ней как по динамическому ключу и перечитывает конфиг при
изменении. `happ_url` всегда с расширением `.json` — `file_extension` относится
только к Outline-артефакту.

Отдельного `config_url` (та же ссылка, но `https://`) в отчёте нет: он
дублировал `outline_url` схемой, а сама ссылка и так лежит первой строкой в
`<user>.txt`. Старый бинарь печатал блок на **артефакт**, а не на юзера — отчёт
никто не парсит, поэтому формат сменили осознанно.

Запись атомарная (временный файл + `os.replace`) — клиент не поймает полуфайл.
Артефакты несут пароль и `vless_id`, поэтому создаются сразу с правами `0640`
(`root:www-data`: раздающая группа читает, остальные — нет), а не world-readable
`0644`; временный файл открывается тем же режимом, короткого окна нет. Каталог
создаётся `0750`. `.tmp`-остаток после сбоя nginx не отдаёт — dotfile-раздача
закрыта (`location ~ /\.` в `nginx-subscription-headers.conf`).

**Группа `www-data` выставляется явно, и это не мелочь.** `0640` защищает
только вместе с правильной группой: файл, оставшийся `root:root`, worker'у
nginx недоступен, и каждая подписка отвечает `403` — причём с момента
перегенерации, а не с момента правки, так что узнают об этом клиенты, а не
прогон. Поэтому `write_atomic` делает `fchown` на группу `www-data`, а если
её нет или прогон не от root — печатает в stderr предупреждение с прямым
указанием на 403, вместо того чтобы записать нечитаемый файл молча.
...
Пользователи — чистые креды: `id`, `password`, `vless_id`, `method`. Своих
путей у них нет — носители задаёт глобальный список `[[endpoint]]` со строками
`{path, kind, padded}`, и юзер работает на каждом эндпоинте своего типа: с
`password` — на всех SS-эндпоинтах (`ws_ss*` / `xhttp_ss*`), с `vless_id` — на
всех VLESS-эндпоинтах (`ws_vless` / `xhttp_vless`). `method` берётся на юзера:
своё значение в `[[users]]` бьёт глобальное из `[shadowsocks]`. Так же это
разрешает сервер.
...
`[outline.probe]` в профиле повторяет группу `main` на cloud1: DNS-проба на
`1.1.1.1` плюс TLS-рукопожатия через туннель к тем же пяти хостам, что проверяет
парк, и `endpoint_check` — сплошной bare-TCP обход всех wire'ов, срабатывающий
только после провалившегося цикла (выключенный exit тогда объявляется мёртвым
сразу, а не через минуты обхода носителей).

Без проб — а именно так профиль генерировался раньше — `healthy` пишет только
data plane, и клиент, у которого разом отвалились все аплинки (смена SIM,
мёртвая сота), не имеет НИ ОДНОГО независимого способа узнать, что они
вернулись; заодно интерфейсу нечего показать в качестве латентности. Это и
приводило к вечному «no link» на телефоне.

Одно осознанное отличие от cloud1 — интервал. Парк пробит каждые 10 с от сети и
по широкому каналу; телефон заплатил бы за это батареей и — на тех самых
edge-каналах, ради которых всё затевалось, — той полосой, которой не хватает
туннелю. 60 с это нижняя граница, которую рекомендует сам ws-rust (ниже неё
всплески рукопожатий начинают ловить upstream rate limit), и она всё равно
находит ожившый аплинк в пределах минуты, без всякого трафика. `attempts = 1`
по той же причине: `min_failures = 2` уже гасит одиночный флап, а вторая попытка
лишь удваивала бы цену каждого цикла.
...
Паддинг — атрибут эндпоинта (`padded = true`), и решается по правилу «только
падёные, если они есть». Если среди носителей цепочки есть хоть один падёный,
генератор оставляет в `<user>.toml` только падёные wire'ы и пишет клиентский
`[padding] enabled = true`; иначе оставляет все (плоские) и `enabled = false`.
Переключатель клиента глобальный (per-wire override нет), а падинг не
согласуется по проводу — падёные кадры на непадёном пути убивают сессию,
поэтому цепочка держится одного класса. Когда часть плоских фолбэков при этом
отброшена, генератор перечисляет отброшенные пути строкой `warning:`.
...
182 теста, stdlib-only, в сеть не ходят и за пределы временного каталога не
пишут. Весь корпус целиком сверяет `test_generate_keys.py::GoldenCorpusTest`, а
по типам файлов — `test_artifacts.py` (`.conf` + `.txt`), `test_xray_json.py`
(`.json`) и `test_ws_toml.py` (`.toml`).
...
| Узлы | `--node` | Раздача |
|---|---|---|
| `cloud1`, `cloud2` | по умолчанию, пара cloud1+cloud2 | да, вход мобильных клиентов |
| `nuxt`, `nuxt2`, `aeza`, `senko`, `sebek` | свой хост | да |
| `.102`, `.104` | `ss.beerloga.su` | **нет** — nginx там выключен |

На `.102` / `.104` юзер ровно один — служебный `cloud`, через который cloud-узлы
держат обратные аплинки; его креды они берут из своих конфигов, а не из
выложенного файла. Артефакты там генерируются на диск, чтобы ссылка была под
рукой, но не публикуются: для этого пришлось бы поднять веб-сервер на домашнем
адресе. Учти, что `public_host` там `ss.beerloga.su` и порт в ссылках — 443,
тогда как наружу проброшены 5443 (`.104`) и 6443 (`.102`).

На `senko`, `.102` и `.104` нет `rsync` — туда заливать через
`tar czf - … | ssh … tar xzf -`. На `.102` стоит `sshguard`, который банит весь
трафик с IP после серии подключений: работать через
