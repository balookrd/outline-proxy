...
# tunnel (`fwmark 0xa` -> table 110). systemd-networkd deletes every routing
# policy rule it did not create itself whenever it starts
# (ManageForeignRoutingPolicyRules defaults to yes) — and an unattended systemd
# upgrade restarts it. The rule then disappears while the table-110 route and
# the nftables marking survive, so the chain breaks at its last hop: every
# marked connection leaves through the node's own public IP instead of the
# tunnel. Nothing logs an error; the node just quietly stops proxying, which is
# exactly how cloud1 lost its rule on 2026-08-11. Two layers, because either
# alone leaves a window: tell networkd to keep foreign rules, and replay the
# bring-up script if networkd cycles regardless.
write_policy_rule_guard() {
    [ "$NET_POLICY_RULE_GUARD" = "1" ] || return 0

    local conf=/etc/systemd/networkd.conf.d/10-keep-foreign-rules.conf
    local unit=/etc/systemd/system/outline-ws0-routing.service
    local conf_body unit_body
    conf_body="# Generated by ops/provision-node/install.sh from profile $PROFILE_NAME.
# The ws0 policy rule (fwmark 0xa -> table 110) is added imperatively by
# /opt/outline/outline-ws-rust/init-ws0.sh, so networkd counts it as foreign.
[Network]
ManageForeignRoutingPolicyRules=no"
    # Not Documentation=: systemd expects URLs there and logs a warning per word.
    unit_body="[Unit]
Description=Restore ws0 policy routing after systemd-networkd restarts
# Generated by ops/provision-node/install.sh from profile $PROFILE_NAME.
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/opt/outline/outline-ws-rust/init-ws0.sh

[Install]
WantedBy=systemd-networkd.service"

    if [ "$DRY_RUN" = "1" ]; then
        dim "[dry-run] would write $conf:"
        printf '%s\n' "$conf_body" | sed 's/^/       /'
        dim "[dry-run] would write $unit:"
        printf '%s\n' "$unit_body" | sed 's/^/       /'
        return 0
...
phase_verify() {
    ...
    # The rule is what puts marked client traffic into the tunnel. Without it
    # the node still answers, still shows healthy uplinks and still logs
    # nothing — it just proxies straight out of its own public IP.
    if [ "$NET_POLICY_RULE_GUARD" = "1" ]; then
        if ip rule show pref 910 | grep -q . && ip -6 rule show pref 910 | grep -q .; then
            ok "ws0 policy rule present (v4 + v6)"
        else
            warn "ws0 policy rule missing — marked traffic bypasses the tunnel; run /opt/outline/outline-ws-rust/init-ws0.sh"
            failures=$((failures + 1))
        fi
    fi
