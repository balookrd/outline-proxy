...
Пути VLESS не зашиты и берутся **на юзера**: `ws_path_vless` /
`xhttp_path_vless` из записи `[[users]]`, а если их там нет — из глобальной
секции `[websocket]`. Так же их разрешает и сервер
(`UserEntry::effective_ws_path_vless`), и на проде это не теория: у служебных
учёток межузловых аплинков пути свои. Чтение только глобальной секции даёт
подписку, которая молча стучится не туда.

Пропускаются с предупреждением в stderr: юзеры без `vless_id` (только
Shadowsocks) и юзеры, у которых нет ни одного пути VLESS. Если задан лишь один
из двух путей, юзер получает подписку с одним транспортом — четыре ноги вместо
шести (или две), а не сломанные шесть.
