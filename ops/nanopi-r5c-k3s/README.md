...
Замер на пустом кластере (2026-08-04, все три ноды): `k3s-server` — **0.37–0.50 ядра из
...
Мерить надо дельтой CPU-времени, а не `ps %CPU`: первые минуты после рестарта дороже
установившегося режима (стартовый reconcile), и среднее с рождения процесса их тянет.

```bash
P=$(pgrep -x k3s-server); H=$(getconf CLK_TCK)
A=$(awk '{print $14+$15}' /proc/$P/stat); sleep 60; B=$(awk '{print $14+$15}' /proc/$P/stat)
awk -v a=$A -v b=$B -v h=$H 'BEGIN{printf "%.1f%% ядра\n", (b-a)/h/60*100}'
```

Нода-лидер etcd стабильно дороже соседей на ~10 процентных пунктов (замер: 49.9% против
40.3 и 36.5) — она рассылает heartbeat и держит больше watch'ей. Если разрыв
перевернулся, значит лидерство переехало на другую ноду.
...
на этом железе стоил ~6% ядра плюс 70 МБ RSS, а нужен только для `kubectl top` и HPA.
Метрики в этом парке собирает VictoriaMetrics (шаг 13), которая скрейпит kubelet напрямую
и в metrics-server не нуждается. Если `kubectl top` понадобится — убрать строку и
перезапустить, k3s поставит его обратно.

**Чего это НЕ даёт (проверено замером до и после):** сам `k3s-server` не дешевеет —
0.42–0.50 ядра до отключения, 0.37–0.50 после. Экономятся ровно ресурсы отдельного
процесса metrics-server; idle control plane держат raft-heartbeat, watch'и между тремя
apiserver'ами и циклы kubelet, и на них эта правка не влияет. Ждать от неё заметного
падения нагрузки не нужно.
