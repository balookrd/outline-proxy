...
Единственная переменная окружения — `Z2M_WATCHDOG=default`, и она ничего не
дублирует: z2m читает watchdog только из env, аналога в `configuration.yaml`
нет. Координатор висит на serial-to-Ethernet мосту Ebyte EBT3001, и раз в
несколько часов ASH ловит таймаут — z2m пишет `Adapter disconnected, stopping`
и выходит с кодом 2. Без watchdog это стоило рестарта контейнера (31 штука за
8 дней к 2026-08-26) и обрывало лог ровно на интересном месте; с ним z2m
переподключается внутри процесса через 2 с, а лог переживает переподключение.
