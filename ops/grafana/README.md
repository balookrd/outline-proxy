...
править нужно файл в репозитории и копировать на узел, подхват — за 30 секунд,
без рестарта. Для экспериментов в UI — «Save as», копия обычная и редактируемая.
...
**После раскатки Grafana нужно перезапустить** — и это главное отличие от
дашбордов. У дашбордов есть file-provider с `updateIntervalSeconds`, он
перечитывает каталог на ходу. У алертинга такого поллера нет: провижининг
выполняется один раз при старте (`starting to provision alerting` в логе), и
файлы, положенные позже, лежат мёртвым грузом до следующего запуска. Проверено
2026-08-07: файлы легли на 27 секунд позже старта — в БД не появилось ни одного
правила.
